...
def generate_random_window_mask(length=500, min_width=5, max_width=50, min_gap=50, max_gap=100):
    mask = np.zeros(length)
    
    position = 0
    while position < length:
        start = position
        width = np.random.randint(min_width, max(min(max_width, length - position), min_width + 1) + 1)
...
cosine_slow = np.cos(np.pi * freq * time / 3) + 1
cosine = np.cos(2 * np.pi * freq * time) + 1
cosine_fast = np.cos(6 * np.pi * freq * time) + 1

signals = [cosine, cosine_fast, cosine_slow]
