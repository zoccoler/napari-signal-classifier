def make_list_of_coefficients_names(waveletname, max_level_of_decomposition):
    ...
    list_coefficients_names = [
        waveletname + "_cA_" + str(max_level_of_decomposition)
    ]
    list_coefficients_names += [
        waveletname + "_cD_" + str(i)
        for i in range(max_level_of_decomposition, 0, -1)
    ]
...
def get_frequency_bands(decomp_level, sampling_frequency):
    ...
    fcD_decomp_level = [
        (sampling_frequency / (2 ** (decomp_level + 1))),
        (sampling_frequency / (2 ** (decomp_level))),
    ]
    return fcA, fcD_decomp_level


def plot_wavelet_coefficient_decomposition_levels(
    signals_table, waveletname, sampling_frequency, figsize=(15, 22)
):
    ...
    frequency_band_list = [
        get_frequency_bands(max_level_of_decomposition, sampling_frequency)[0]
    ]
    frequency_band_list += [
        get_frequency_bands(level, sampling_frequency)[1]
    ...
    for i in range(signals_table.shape[0]):
        ...
        for j, coef, freq_band in zip(
            range(len(coefficients)), coefficients, frequency_band_list
        ):
