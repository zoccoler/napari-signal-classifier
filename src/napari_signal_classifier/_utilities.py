...
def extract_numbers_with_template(data_list, template):
    ...
    for item in data_list:
        ...
        if match:
            number = match.group(1)  # Get the number part of the matched string
            if number is None:
                number = ''
...
def get_frequency_bands(decomp_level, sampling_frequency):
    ...
    fcD_decomp_level = [(sampling_frequency / (2**(decomp_level + 1))), (sampling_frequency / (2**(decomp_level)))]
...
def plot_wavelet_coefficient_decomposition_levels(signals_table, waveletname, sampling_frequency):
    ...
    frequency_band_list = [get_frequency_bands(max_level_of_decomposition, sampling_frequency)[0]]
    ...
    for i in range(signals_table.shape[0]):
        ...
        # Get coefficients
        coefficients = pywt.wavedec(signal, waveletname, level=max_level_of_decomposition, mode='per')
